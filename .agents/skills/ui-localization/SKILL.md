...
`AdvisorySharedDamageCredit` 正值时单独解释无来源伤害按人数折算参与选路，零值隐藏；不能加到个人已付/计划贡献或达标提示。中英模板参数使用同一小数格式，`shared-ranking` 覆盖三种语言的正数与零值分支。该显示不是队友未来输出预测。
...
搜索读数的逐帧过渡归 `SolverOverlayMotionDriver` / Overlay；内存条、动作胶囊和上传面板只平滑已接收的显示值，不能改变原始统计、搜索预算或完成判定。驱动随覆盖层释放，隐藏后重显同步最新值；多人准备/过期提示不能被旧读数覆盖。窗口保存用户请求位置，屏幕夹取只改变显示位置，鼠标释放由全局输入桥提交持久化。

搜索中速度由进度差值平滑，完成后的平均速度由 snapshot 投影；零速度不显示括号，复用和缓存恢复保留各自摘要。移除重复前沿提示时仍保留路线验证与多人过期状态。内存设置分别说明本次实际模式和下次启动配置，保留重启／卸载前关闭提示；不将历史无头降幅写成玩家可见性能保证。
