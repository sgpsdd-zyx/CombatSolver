...
`codex/multiplayer-advisor` 向自己的 `origin` 发布任务分支、当前版本 annotated tag 和 GitHub Release。官方 `main` 的三渠道脚本及工坊／夸克身份不适用于此分支。按下面相同的准备、冻结和阶段证据规则，使用 Git 原子推送当前分支与当前标签，再以 GitHub CLI 创建 Release、上传已定版的最小 ZIP 并用 `--notes-file` 提供完整中英玩家日志。显式指定当前 fork 仓库，使用 `--verify-tag`，不自动生成上游发布说明。

复用已有成功构建与 ZIP；标签指向对应 release source commit，纯文档归档提交可以位于其后。发布成功后只补记渠道凭证、同步当前版本入口并提交推送文档，不复测、重打包或重新读取远端确认。以下涉及创意工坊、夸克及统一脚本的要求只适用于官方 `main`；多人 fork 未上传这两个渠道不记为发布失败。
...
| `发版/发布` | 补齐准备阶段、创建 annotated tag 并发布适用渠道；多人 fork 使用 GitHub，官方 `main` 使用三渠道统一脚本 | 完整门禁、监控后台版本提示 |
...
官方 `main` 正式发版通过 `tools/publish-release.ps1` 同步发布创意工坊、GitHub Release 与夸克网盘。`准备发版`、普通开发提交和单独推送源码不运行统一发布脚本；只有进入 `发版/发布` 阶段时执行。多人 fork 使用本 skill 首节的 GitHub 路径。监控后台的 `latestVersion` 由用户维护，agent 不读取或写入。
...
官方 `main` 正式发版只使用 `tools/publish-release.ps1` 作为三个渠道的执行入口。外部服务无法组成原子事务，脚本按创意工坊、GitHub、夸克网盘顺序执行，并在 `releases/CombatSolver-<版本号>.publish-state.json` 记录成功阶段；失败后从未完成阶段恢复，不重复已经成功的渠道。
...
官方 `main` 正式发版由第 5 节统一脚本创建 GitHub Release；多人 fork 遵循首节。两者均上传已定版最小 ZIP 并提供完整中英玩家更新日志。仅开发、暂不发版和单独推源码仍不创建 Release。已有同版本成功状态时直接复用，不重复创建或上传。
