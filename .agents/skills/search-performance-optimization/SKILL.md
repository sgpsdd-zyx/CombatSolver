...
多人使用独立本机贡献目标，不预测队友主动行为。Runtime 从阶段起点冻结有效敌人 HP / 存活人数与三敌方周期绝对截止，手动重算/治疗/额外玩家回合不刷新；人数变化明确重建并保留旧结果。删除每周期 3 HP 和 A/C 覆盖升级，当前机制见 `docs/multiplayer-advisor.md` 与贡献策略实施记录。

多人实验以只有本机使用军师为前提。队友只用自己的当前手牌/资源和公开局面独立选牌，不读取本机候选或被要求配合；`tools/MultiplayerExperiments` 通过原生无头驱动逐项交错并模拟本机手动重算。比较前核对全队根、完整有效政策和允许消融字段；A/A 还检查每层时间截止，最终 NodeLimit 不代表全程未受时间影响。原生终局与限额/失败分开，生命变化分战斗内与战后，受控模板不计为独立真人来源。测试传输、教学与远端动画绕过不得用来证明真实网络和可见性能。

无来源的实际敌方扣血单列 `UnattributedDamage`，按阶段人数折算后统一用于中间分、阶段缺口与条件尾值；本机 `Progress` 和达标见证仍只认可真实归属。治疗冲回、人数重建不重复领取信用。前沿支配须同时核对多人目标代价，去重/转置不能合并不同无来源计数。使用 `shared-ranking`、`race-dev`/`race-holdout` 与原生 `MULTIPLAYER-SHARED-DAMAGE` 检查；原型队友死亡折价/威胁压力已撤回，历史均速预测和 ready 不可撤销假设未采用，见 `docs/strategy/multiplayer-shared-damage-20260923/implementation.md`。

一次搜索复用原 Beam/预算，保留至多 `4B` 的伤害/生命资源代价前沿。用药资格先过滤，比较周期从截断前池冻结，缺证据不能当零来伤或不可达；风险候选不能支配可靠候选。阶段内真实死亡与胜利照实处理，阶段后的本机条件续行只作有折价尾值，不要求独力清场。长线收益必须来自真实推演，不能按未知队友 DPS、易伤层数或深度本身加分。阶段末动作数来自父链，终局用真实完成事实。

多人普通时间/节点预算乘二、固定预算不变，最多十四敌方周期；额外玩家回合不消耗周期席位。中间保路覆盖防御、进攻、铺垫，不扩大 Beam 或 No-GC。伤害归属计数与原始检查点进入去重/转置标签及增量对账，不能进战斗键或 ContinuationStamp。跨请求只保留阶段已观察事实和有界纯动作前缀，旧分数/分支不能续用。`quota-contracts` 覆盖阶段、反击、归属、Fork、预算未知与 UI；`quota-selection` 覆盖资格、固定排序和前沿；成长压力/低血防御/投资短搜与官方单人哨兵作为质量边界。历史 `window-*` / `horizon-ordering` 须在 0.43.6 标签复现。
...
- 单人主结果战损下界只在根牌组、遗物、Power、药水与扩展来源落入 `StrategicHpRecoveryBound` 已核对的无治疗封闭集合时收紧；固定战后遗物回血仍计入。未知来源、治疗牌、生成牌、可重复回血及药水沿用完整缺血余量。多人根不捕获该单人证书，未来回血额度保持完整缺血余量，主结果剪枝入口仍提前返回。精神过载在下个玩家回合开始施加灾厄，灾厄致死只在该玩家回合结束结算；不能将当前灾厄值当作提前损血或死亡下界。
...
- Steam 自动 GC 接入只准备下次启动：`RuntimeGcStartupConfig` 验证普通安装或 macOS 同一 app 内的路径，原子更新 runtimeconfig 并记录／恢复原 GC 字段；该层不依赖 Godot、设置或搜索。`RuntimeGcProfile` 只以启动 AppContext 与实际 CLR 状态生效。默认无头测试不改配置，验证必须在自有隔离实例显式启用；不将文件已写入、普通 .NET 子进程或无头验证当作可见 Steam 链路通过。直接退订无法运行恢复逻辑，须明确披露并提供关闭入口。Windows/Linux 三启动脚本与 macOS 原生入口各保留相同启用／恢复边界，当前平台以外未运行项明确记录。
