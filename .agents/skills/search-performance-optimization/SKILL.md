...
多人军师使用独立窗口目标和比较器，默认最多十四个敌方周期，不套用单人的药水省血审计与局外收益。用户已授权增加多人计算额度：`MultiplayerSearchPolicy.ResolveSearchProfile` 在协调器多人入口把普通请求时间/节点上限乘二，固定预算或显式时间覆盖保持原值；不改设置、Beam、分支、并行度或 No-GC 额度。`Phases` 按前沿最浅的已完成敌方周期计算剩余周期分摊，额外玩家回合不提前消费周期席位；保留 250ms/500 节点最低份额及全局截止。结果分别报告上限、实际推演周期和有效时间/节点预算，UI 只投影这些值。十四周期不保证完成，也不改变短路线共同周期回退。验证复用 `horizon-fourteen`、`horizon-budget` 与 7/14 周期原生合同，记录见 `docs/strategy/multiplayer-fourteen-cycles-20260920.md`。跨请求只保留有界纯动作前缀，在新根上逐步验合法性并重新模拟；不能复用旧分数、旧分支或用单人资源摘要支配队友状态不同的分支。复用成本计入本次预算，收益须实测；详见 `docs/multiplayer-advisor.md`。
...
多人终局事实优先于旧检查点，但不改变原风险顺序；本机无损满血胜利捷径仅属于单人。多人根前用药资格从 Runtime 的原追踪窗口按本机 Actor 冻结，不在 Search 重新读取历史。修改这些入口时使用 `OfflineSearchHarness --multiplayer-review-contracts facts`；只复核早停时用 `stopping`，观察实际展开父节点的回合，不能用最终路线的 `SearchedTurns` 推断搜索覆盖。联合完成池压缩与资源归属的后续范围见多人指南，不把研究清单全部视为实施要求。
