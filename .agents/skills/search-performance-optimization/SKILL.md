...
多人输出目标允许每个敌方周期最多 3 HP 扣血，先规避死亡和超额扣血，再比较输出；同等输出少扣血。`SearchNode.AdvisoryHpLoss` 的 `MultiplayerHpLossBudget` 记录不可跨周期结转的额度，根包含本轮已付扣血，治疗和额外玩家回合不重置。原始周期末检查点在下一回合准备前捕获；同战斗状态但剩余额度不同的节点不能被去重或转置错误合并。多人 `BeamRetentionPolicy` 分片在原宽度内保留防御、进攻和铺垫路线，不借用单人 Pareto。验证至少包含 1/2/3 HP 换输出、超过 3 HP、低血量、同输出少扣血，以及下一回合自损归属。
