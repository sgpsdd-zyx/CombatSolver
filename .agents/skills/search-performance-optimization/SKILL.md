...
多人军师使用独立窗口目标和比较器，默认最多七个敌方回合，不套用单人的药水省血审计与局外收益。窗口变长不增加节点/时间预算，结果分别报告窗口上限与已推演的敌方回合数。跨请求只保留有界纯动作前缀，在新根上逐步验合法性并重新模拟；不能复用旧分数、旧分支或用单人资源摘要支配队友状态不同的分支。复用成本计入本次预算，收益须实测；详见 `docs/multiplayer-advisor.md`。
...
有共同周期的未终局路线只比较该周期之前的动作成本；`MultiplayerActionCountAt` 从既有父链找精确跨周期动作，不按玩家回合号猜测、不为缺失边界返回默认值。完整终局继续比较完整动作，未完成首周期保留原后备。后缀动作不能成为奖励或惩罚，后续已知风险继续生效。修改此口径时运行 `--multiplayer-review-contracts horizon-ordering`，覆盖后缀牌、后续周期、前缀成本、已释放模拟器和固定上下文传递性。
