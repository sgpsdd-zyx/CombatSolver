...
`MultiplayerCycleCheckpoint` 仅保存敌方周期结束、下一玩家准备前的原始数值；由不可变短链引用前周期，Fork 可共享旧记录，子分支追加不能回写父分支。它不持有 Model 或模拟器，也不决定战斗语义；政策历史保留于搜索标签。变更捕获时点须验证开始阶段自损归属、额外玩家回合不追加周期、完整原生状态和 Fork 隔离。
