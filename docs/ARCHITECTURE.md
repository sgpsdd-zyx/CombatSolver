...
状态键的历史依赖由主线程 `CombatRootSnapshot.HistoryDependencies` 冻结：全部本机战斗牌（含消耗堆）按读者选择六类计数中的必要项；活动随机生成、变牌、间接生成药水、可能独立于原卡存活的 Nightmare 保存副本以及第三方模型/订阅者/OnPlay 适配保守使用全部六项，覆盖未来读者。`CombatHistoryCounterKey` 的开放生成来源表须与新增原版生成入口同步；固定衍生牌与复制现有牌不引入新的读者类型。单人分支读取增量历史，多人仍扫描历史并保留各计数原有的全队/持有者范围；两者使用不可变根掩码，根之前的历史恒定。
...
搜索中的速度由 Overlay 对已接收进度作差、平滑并逐帧显示；完成后的平均速度由 snapshot 从该次搜索耗时与工作量投影，缓存恢复与复用保留原摘要。速度不反馈给搜索预算或策略，重复前沿提示精简不删除路线验证与多人过期状态。
...
多人遗物注释从 `ActionRelicTriggerRecorder` 保留 `OwnerNetId`，去重包含持有者，不能合并不同玩家的同名效果。主线程 `SolverDisplayNames` 冻结全队遗物名称及玩家编号；最终路线将编号和本机标志写入 `PlanRelicEffect`，UI 投影到 `SolverRelicTextIdentity` 后显示自己/队友编号。归属随 JSON、路线身份比较和语言刷新保留，不作为战斗状态或搜索评分；单人不写附加字段，保持原有提示。佩尔之眼的多人参与标志仍属于既有逐遗物分支状态，回合开始必须更新未参加者；不得用 UI 归属替代实际结算修正。
