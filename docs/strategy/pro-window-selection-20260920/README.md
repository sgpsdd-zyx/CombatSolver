...
日期：2026-09-20。用户要求继续借助此前的 ChatGPT 6 Pro，研究如何在队友动作未知的多人战斗中兼顾战损与长线输出，避免未击杀候选只按第一周期收益排序。固定研究输入为 `851c1521f5258ec43dc71c24eeb8d5200fdf612d / 0.43.3`。用户随后授权实施，当前采用状态和验证见[0.43.4 实施与知识收尾](implementation.md)。
...
上述为研究完成时的状态。随后授权的 0.43.4 采用受限 C：覆盖不足等条件回退 A，只有 C 周期确实提高时使用共同周期动作成本；十四周期上限与原风险顺序不改。新增实际对照、未验证范围和本地准备状态统一维护在[实施记录](implementation.md)，6 Pro 原文不改写。
