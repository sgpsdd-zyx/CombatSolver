...
- [多人搜索窗口研究与实现](pro-horizon-20260919/README.md)：保留默认七周期、不启用自适应；归档原始研究、24 组生产对照与已验证的共同周期动作成本修正。
...
R1/R2 已在 0.41.2 修正。用户随后授权实施，本地已完成 F01 终局整体事实、F02 多人早停隔离和 F03 本机根前用药输入；[实施与知识收尾](pro-review-0412-20260918/implementation.md)记录本轮真实失败/修正证据及条件项暂缓原因，改动记入下一版本开发批次，尚未发布。Pro 撤回“两步挑战优先”，本批采用保留现役 Beam 的低成本回退。联合候选压缩、资源归属观察等仍待后续小批次；资源风险排序是独立政策决定，抽象实验不证明实际联机收益。
