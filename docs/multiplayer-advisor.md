...
本 fork 从 `0.40.3` 起提供多人建议模式；单人仍使用原有搜索政策和自动执行入口。用户在实现及局部模拟验证完成后，授权推送至 `sgpsdd-zyx/CombatSolver` 并发布可直接安装的 GitHub Release。安装与玩家说明见 [版本日志](releases/0.40.3-RELEASE_NOTES.md)。最初的本地玩家规划文档属于设计输入；本文描述实际实现和验证范围，多人功能尚未完成真实联机验收。
