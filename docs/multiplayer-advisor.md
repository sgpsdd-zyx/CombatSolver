...
本 fork 从 `0.40.3` 起提供多人建议模式；`0.40.4` 修复首次手动计算被旧单人记录限制阻断的问题；`0.40.5` 改为在单回合 3 HP 扣血目标内优先输出。本次以 [0.41.1（fork）](releases/0.41.1-RELEASE_NOTES.md) 定版：基于官方 `0.41.0`，包含 `6e28485` 的保路配额、共同周期比较和有效救援三步优化；单人继续使用该官方版本的搜索政策和自动执行入口。官方 `v0.41.0` 标签不包含后续多人优化；本 fork 的对应源码使用 `v0.41.1` 标签。原始研究与历史版本由[策略索引](strategy/README.md#多人研究归档)归档，本文是现役行为与采用范围的唯一入口。多人功能尚未完成真实联机验收。
...
| `Search/MultiplayerSearchPolicy.cs`、`CombatBeamSolver.Multiplayer.cs` | 多人目标、节点扣血账本、多人 Beam 保路和受限旧路线重放 |
...
`RankFinal` 与最终选路各自从收到的候选组确定最浅的已完成周期，再在该阶段的固定边界比较整批。真实提前胜利/死亡是终局，外部选择和未支持边界不是；无法完成共同周期的路线不与已完整受击评估的路线冒充同等证据。全部候选都被阻断时仍可报告最后观察及其边界。比较器不会为每一对候选临时换深度。

0.41.1 仍有两个选路管线限制：最终截断后才检查“至少用一瓶/最少显式用药”的资格，可能提前丢弃合格路线；截断和过滤后的子集会重建共同周期，尚未贯通同次选路的比较上下文。[复审与本地核对](strategy/pro-review-20260918/local-review.md)给出调用链与候选级反例。当前没有生产管线复现或游戏触发频率证据，研究归档未修改这些行为。
