...
完成池局部压缩与最后联合比较仍可能使用不同候选集合；末父节点的已付卡牌候选、共同周期外动作数同分及旧当前回合预览旁路也仍有研究期限制。默认全队救命次数与风险次序保持原样。后续条件、未验证范围和暂缓理由集中见[实施与归档记录](strategy/pro-review-0412-20260918/implementation.md#保留的条件项)，不能把首批修正理解为所有入口已完备或联机策略已最优。
