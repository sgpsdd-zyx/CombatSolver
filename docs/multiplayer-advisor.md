...
按用户确认的多人打法，每个敌方周期以本机扣血不超过 3 HP 为目标，优先避免本人死亡、消耗复活效果和超额扣血；同等本人风险下优先真实胜利和保住队友，再压低敌人剩余生命。输出相同时少扣血，相同收益时少用药。没有满足额度的路线时仍会给出超额较少的建议。路线比较使用已经完成的共同周期，搜索更深本身不加分；具体续行后来出现的死亡、复活消耗、超额或队友死亡仍作为反证，不能藏回较早的安全检查点。
...
`RankFinal` 与最终选路使用同一批次规则：先从可继续的非终局候选确定最浅的已完成周期，再在这个固定边界比较整批。真实提前胜利/死亡是终局，外部选择和未支持边界不是；无法完成共同周期的路线不与已完整受击评估的路线冒充同等证据。全部候选都被阻断时仍可报告最后观察及其边界。比较器不会为每一对候选临时换深度，避免不传递排序。
...
共同边界内依次考虑强制药水约束、本人实际死亡、证据完整性、复活消耗、逐周期超额、真实胜利、队友存活、敌人生命、本人扣血/余血和药水。同一路线在更后面已经发生的死亡、复活消耗、超额或队友死亡仍参与风险判断；不能用它较早的安全观察掩盖。这个反证只否定具体续行，不封禁同首动作的其他合法路线。尚未展开的未来仍然未知。
