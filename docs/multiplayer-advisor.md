...
本 fork 从 `0.40.3` 起提供多人建议模式；`0.40.4` 修复首次手动计算被旧单人记录限制阻断的问题；`0.40.5` 改为在单回合 3 HP 扣血目标内优先输出。当前已发布版为 [0.41.3（fork）](releases/0.41.3-RELEASE_NOTES.md)，基于官方 `0.41.0`；在此前保路配额、共同周期比较、有效救援和用药资格修复基础上，纳入[第二轮复审首批](strategy/pro-review-0412-20260918/implementation.md)的终局事实、多人早停和本机根前用药修正。共同周期动作成本调整仅留离线实验。单人继续使用官方 `0.41.0` 的搜索政策和自动执行入口。官方 `v0.41.0` 标签不包含 fork 优化，当前源码使用 `v0.41.3`。原始研究与历史版本由[策略索引](strategy/README.md#多人研究归档)归档。多人功能尚未完成真实联机验收。[长线收益诊断](strategy/pro-long-term-20260918/README.md)已归档；它不改变默认多人策略或单人路径。
