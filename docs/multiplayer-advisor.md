...
本 fork 从 `0.40.3` 起提供多人建议模式；`0.40.4` 修复首次手动计算被旧单人记录限制阻断的问题；`0.40.5` 改为在单回合 3 HP 扣血目标内优先输出。最新已发布的 [0.41.1（fork）](releases/0.41.1-RELEASE_NOTES.md) 基于官方 `0.41.0`，包含 `6e28485` 的保路配额、共同周期比较和有效救援三步优化；单人继续使用该官方版本的搜索政策和自动执行入口。当前任务分支另有未发布的资格过滤和同次选路共同周期修正，见下方采用记录。官方 `v0.41.0` 标签不包含后续多人优化；已发布 fork 的源码使用 `v0.41.1` 标签。原始研究与历史版本由[策略索引](strategy/README.md#多人研究归档)归档，本文是现役行为与采用范围的唯一入口。多人功能尚未完成真实联机验收。
...
0.41.1 曾在截断后才检查“至少用一瓶/最少显式用药”，并在子集上重建共同周期。[复审与本地核对](strategy/pro-review-20260918/local-review.md)保留原始发现；2026-09-18 按用户指令实施第一批修正，记入下一版本开发记录。已终止的候选在中途压缩时也先检查资格；仍能继续展开的未用药前缀不因最终要求被提前排除。没有合格结果时继续明确报告用药条件未满足。资源归属、默认风险顺序与两步挑战不属于这次修正。
