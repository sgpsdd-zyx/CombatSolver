...
本 fork 从 `0.40.3` 起提供多人建议模式；`0.40.4` 修复首次手动计算被旧单人记录限制阻断的问题；`0.40.5` 改为在单回合 3 HP 扣血目标内优先输出。当前开发源码基于官方 `0.41.0`，在 `6e28485` 落实保路配额、共同周期比较和有效救援三步优化；单人继续使用该官方版本的搜索政策和自动执行入口。2026-09-18 的归档与远端同步面向源码，未定版新的 fork 安装包；此前本地准备包说明见 [版本日志](releases/0.40.5-RELEASE_NOTES.md)。原始研究与历史版本由[策略索引](strategy/README.md#多人研究归档)归档，本文是现役行为与采用范围的唯一入口。多人功能尚未完成真实联机验收。
