...
本 fork 从 `0.40.3` 起提供多人建议模式；`0.40.4` 修复首次手动计算被旧单人记录限制阻断的问题。单人仍使用原有搜索政策和自动执行入口。用户授权推送至 `sgpsdd-zyx/CombatSolver` 并发布可直接安装的 GitHub Release，安装与玩家说明见 [版本日志](releases/0.40.4-RELEASE_NOTES.md)。最初的本地玩家规划文档属于设计输入；本文描述实际实现和验证范围，多人功能尚未完成真实联机验收。
...
按用户最后追加的 5～7 回合需求，搜索默认最多覆盖七次敌方行动周期；额外玩家回合不消耗这个窗口。当前可以击杀时优先击杀，否则比较窗口内的生存、累计扣血和剩余敌人生命；相同收益时少用药。节点/时间预算不因窗口扩大而增加，预算不足时可以提前返回。面板同时显示实际推演的敌方回合数、上限和相应边界提示，不保证每次达到七回合。Beam 是有界启发式搜索，不承诺全局最优。
...
| `Runtime/CombatRootSnapshot.cs`、`ContinuationStamp.Multiplayer.cs` | 主线程冻结、完整多人状态对账 |
| `Search/MultiplayerSearchPolicy.cs`、`CombatBeamSolver.Multiplayer.cs` | 多人目标、结果比较和受限旧路线重放 |
| `Search/CombatBeamSolver.MultiplayerRound.cs` | 全队回合阶段、额外回合、敌方周期及本人的选择展开 |
| `Search/SimulatedCombatState.Multiplayer.cs` | 本人身份、额外回合参与者、窗口计数、队友选择边界、历史窗口 |
