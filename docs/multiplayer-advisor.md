...
2026-09-17 对用户提供的 [策略研究原文](CombatSolver_Multiplayer_Strategy_Research_and_Design.md)进行了本地源码复核。行为基线仍为 `f220a6b / 0.40.5`；后续文档提交未改变算法。本节是实施取舍和验收计划，原文中的算法名、伪代码、实验数值与 P0～P7 均不代表已经实现或验证。
...
实施时先保存 `f220a6b` 行为基线的目标输入和结果；每步只补一条代表性失败基线及对应最终证据。第一步验证覆盖，第二步验证浅好/深差与浅好/后续致死两个方向、提前胜利、额外回合及开始阶段自损，第三步验证怪物打队友、有效救援和可兑现铺垫。使用实际 `0.111.0` 模型构造输入，不把研究中的抽象例子或 Python 数学断言当作游戏验收。
...
本次评审只更新研究索引、本文和项目 skill，尚未修改策略源码或运行游戏测试。`neat-freak` 安装在项目 `.agents/skills/neat-freak`，用于检查本次涉及的文档与规则是否符合实际；安装来源和精简范围见 [UPSTREAM.md](../.agents/skills/neat-freak/UPSTREAM.md)。运行态与联机质量仍待后续行为验证；发布、部署、跨项目清理和生成记忆不属于本次评审范围。
