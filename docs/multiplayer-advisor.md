...
本 fork 从 `0.40.3` 起提供多人建议模式；`0.40.4` 修复首次手动计算被旧单人记录限制阻断的问题；`0.40.5` 改为在单回合 3 HP 扣血目标内优先输出。当前定版为 [0.41.2（fork）](releases/0.41.2-RELEASE_NOTES.md)，基于官方 `0.41.0`，包含此前保路配额、共同周期比较和有效救援优化，以及 `b29d6fc` 的资格过滤和同次选路共同周期修正；单人继续使用该官方版本的搜索政策和自动执行入口。官方 `v0.41.0` 标签不包含后续多人优化；本版本源码使用 `v0.41.2` 标签。原始研究与历史版本由[策略索引](strategy/README.md#多人研究归档)归档，本文是现役行为与采用范围的唯一入口。多人功能尚未完成真实联机验收。
...
0.41.2 的[第二轮复审与本地核对](strategy/pro-review-0412-20260918/local-review.md)发现现役实现仍有边界缺口：真实胜利可能先命中旧检查点而丢失终局事实，本机无损满血胜利仍可能触发不充分的多人早停，根前队友用药历史也可能放宽本机用药要求。完成池局部压缩与最终联合目标另有待验证丢路例。这些发现尚未修复，不能把上述目标顺序理解成所有入口均已完备；实际搜索/联机可达性与已复现的生产函数反例分开记录。
