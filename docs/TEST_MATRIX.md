...
- 已安装官方 0.46.2 DLL 的五项客户端连接元数据通过存在性、HTTPS 和证书指纹格式检查，仅写入忽略目录的私有 props。`02d1523c` 的正式 Release 构建 0 警告/0 错误，10.22 秒；新 DLL 连接元数据检查通过，最小 ZIP 与本地部署成功，分支/标签原子推送及 GitHub 最新版创建成功，见[渠道凭证](releases/0.46.3-PUBLISH.md)。
...
- 单人 `solo-power-compat` 与独立官方0.45.0：92非时序字段、根/完整路线/续行一致，1750展开/4363转移/2HP，无时间边界，四个多人入口0次。固定Beam12/350节点/10000ms/DOP1，Coordinator及portfolio。
- 托管合同：回合开始登记22项/冻结1项，抽牌后登记41项/冻结3项，Hook掩码61位；多人兼容13项、共享选路16项、死亡/防死/复活/下一回合全队状态及3次手动搜索通过。托管原生方法调用不代表真实网络或渲染。
- L1 原生：`LAMP-DEBUFF-ON-KILL` / `macos-6ca53edaca6e485da508aaf29e730eb3`、`LAMP-POWER-SOURCED-DEBUFF` / `macos-15c2c702c2d64045b06c26ea18de4f7c`、`LAMP-INDIRECT-POISON` / `macos-07312260279144759706e3b59b5bfdba` Passed，完整ContinuationStamp差分。
- L2 原生：`MULTIPLAYER-SHARED-DAMAGE` / `macos-737a459d34d84e38bddc9e696adce840` 21项，包括生产回合登记入口次序、完整原生周期及共享账本、混毒、Fork、ready撤销和严格增量。
- 原生UI：`UI-LOCALIZATION` / `macos-70a619a0faa143248fd28e50c0ffcc8f` 28条记录，459模板/英简繁/原生站位/循环高亮与重用。`UPSTREAM-0450-UI-STATE` / `macos-43edb7dc21ff41669b234de3a423e583` Passed，窗口位置写盘/重载/释放/夹取恢复、内存条及控制器会话；最小输入 `coverage/unattended/upstream-0450-ui-state.json`。
- L2 手动流程：`macos-a1a046bfab084d3e9aa9ced0a9d1f1f1` 4次查询、本机/队友各3动作、1次严格全队动作差分、6次冻结根检查，进入第二回合；同进程随后 `macos-5747a2147aa64c4696ff94cc92c06ef8` 确认实验钩子清空与单人一牌胜利建议。
- 两次失败保留：UI首次错用Fuzzy导致四怪站位初始化报错；窗口首次在断言后继续等待初始搜索，120秒超时。分别修正遭遇和停止边界后通过，没有修改生产语义或延长超时。3个原生实例均由启动器删除。
- 合并版/官方/工具构建零警告错误；CoverageCatalog `--verify --verify-effective --verify-branch-state-reads` Passed，3035条目、0未分析/未分类状态/精确镜像live分支读取，22个根快照Hook在回放范围外。Bash职责门禁218文件通过，PowerShell同步未执行。最后交付构建和渠道凭证独立归[版本目录](releases/README.md)。
...
- `quota-contracts` 32项通过：阶段/治疗/人数变化、实际过杀、本机索引1、Fork隔离、队友及未知伤害不记个人贡献；本机原生出牌与真实敌方周期的全队 ContinuationStamp 对照相同。目标达标仍执行攻击与防御，严格增量回放核对完整状态和新增归属/检查点字段；极低预算保持 Unknown，终局不倒填阶段期限。
- `quota-selection` 用药资格、固定比较周期、4B前沿、840种候选排列及216组三元比较通过；`facts` 10项通过。退役 `window-*`/`horizon-ordering` 合同仅属于v0.43.6，不计为当前结果。
- 固定Beam12/1400节点/3000ms的压力、防御、投资短搜分别326/48/395展开，639/86/1096转移，均未触碰时间上限。低血例首动由旧版攻击变防御，2周期6伤害/0损失；第三周期未证实。压力/投资首动与前三周期伤害战损和旧基线一致。独立 Fogmog 留出两根记录52/297展开，仅为观察，无旧版同根提升结论。
- 七根×H7/H14共14次固定350节点/3000ms外部统一续行：晚回本根H14选择能力，伤害84→104且0损失；H7仍攻击。更晚回本三根仍选择攻击，是已知局限。队友扰动脚本只在外评侧，不进入规划器。
- 单人同根与独立官方0.44.0保留基线的71项非时序字段及完整根/路线/续行一致，1750展开/4363转移/2HP；四个多人入口零次。后续只改多人政策，不重复单人哨兵。
- 原生 `UI-LOCALIZATION` / `macos-867c13c432e445b0a73e34343c142c52` Passed，458模板、eng/zhs/zht。无头实例 `macos.3LkD94`由启动器删除；Bash职责门禁 `search_files=218`，PowerShell规则同步未运行。
- CoverageCatalog定向 `--verify-state-fields --verify-branch-state-reads` 通过：0未分类字段、0精确镜像分支可变live读取。工具引用和新增带限制状态兼容性已修正；仅此定向检查，不冒充全量语义复测。
- 生产与离线宿主 Release 零警告错误；最终版本/文档变更仅作一次定版构建，不重跑已通过行为。原始证据 `.local/multiplayer-quota-20260922/`，见[实施记录](strategy/multiplayer-cooperative-planning-20260922/implementation.md)与[结构化摘要](strategy/multiplayer-cooperative-planning-20260922/implementation-evidence.json)。
- 未执行真实联机、可见性能、全角色、四算法完整消融、胜率研究、干净安装或完整发布门禁；本次不宣称最优或全场正收益。上一合并阶段未完成项保持未验证。
...
以下新增“上游历史”导入官方 `68700c23 / 0.45.0`，仅保留贡献者原始记录；上游平台测试和视觉观察不算本 fork 本轮验证。当前合并证据另列。

## 上游历史：0.45.0 定版验证范围（2026-09-23）

- 本次仅同步版本与玩家更新日志，行为源码沿用下列 PR #130 Windows 集成和位置持久化成功证据；发布构建从最终提交执行，不重复相同行为场景。

## 上游历史：求解器窗口位置持久化（2026-09-23）
...
## 上游历史：能力驱动的 Power 施加不再继承外层卡牌来源（问题包 c4e28f3b）（2026-09-22）
...
## 上游历史：击杀后不再向已离场个体施加 Power（问题包 24b8f299）（2026-09-22）
...
| 检查 | 本轮直接证据与范围 |
|---|---|
| 现役生产比较函数 | `ordering-probe-results.json`：复用发布 DLL，十项合成节点断言通过；覆盖超额优先、真胜利控制、团队人数、输出、估值同分和已知深后缀风险。没有游戏根、候选生成或实际牌序；探针构建有一条 `CS8625`，原始输出保留 |
| 配额最小反例 | 四组结果：真实 HP 下攻击+格挡与缩血假胜利；直接输出与易伤团队收益；有限集合前沿复用；Unknown 被误当不可行的二分。两组完整一回合枚举，另两组纯数学构造，非生产搜索 |
| 滚动期限 | `deadline-results.json`：有限排程完全枚举。每次重置未来三周期，三次只防输出 0；固定阶段期限执行防攻攻、输出 20。仅证实该拖延反例 |
| 6 Pro 外部抽象 | 完整原始 ZIP：780 主对照 / 60 根 / 13 算法，126 预算对照，3 窄 Beam，27 检查；失效 v1 不计有效数。H/Q 曾用于选型，R 八根为最终未调参集，非真人分布。原脚本和原始结果已收取，本地未重跑 |
| 外部数据提取 | 909 运行键唯一；各行逻辑费用和等于 `used`、不超声明上限。R 的 HP、总敌 HP、损失与资源指标相同，R02 逐敌向量不同；首次过强的向量相等断言失败后修正口径，未修改或重跑外部实验 |
| 6 Pro 配额专项 | 独立后续响应 `Worked for 18m 50s`，原文已收取；修改推荐顺序、撤掉软 3 HP。新配额/F0/参数与硬门槛反例是未运行设计或直接推导，不增加实验数 |

本地源/结果归档见[探针说明](strategy/multiplayer-cooperative-planning-20260922/local-probes/README.md)，外部精简账本与原始完整资料位置见[6 Pro 入口](strategy/multiplayer-cooperative-planning-20260922/pro/README.md)。配额/前沿生产方案尚未实现；首轮外部 F_cheap 不是配额算法。外部评估只固定一次当前动作再用统一脚本推进，不能证明连续重算整场质量。

L0 检查覆盖 21 份 Markdown 的 534 个本地路径、7 份 JSON 解析及 3 份 Python 语法，均通过；凭证为 `.local/mp-coop-strategy-20260922/doc-validation.json`。暂存差异空白检查仅报告原始 Pro 补充第 3、4 行的两个 Markdown 硬换行，按原文保留，不能记作无告警通过。

本轮没有产品构建、原生 actual/simulated、真实多人网络、全角色/遭遇、可见性能、包或发布验证；旧行为测试不作为本轮新通过。

后续按用户要求取消队友预测，只修订设计及待执行矩阵：移除主动队友策略与对应语义扩展，保留本机长线、被动状态、配额及手动重算。旧实验数量不变，未新增算法或游戏验证；本次 12 份改动文档中的 11 份 Markdown、528 个本地路径、1 份 JSON 与差异空白检查通过，独立记录在 `.local/mp-coop-strategy-20260922/no-peer-prediction-doc-validation.json`，不覆盖首轮凭证。

## 0.43.6（fork）：GitHub 发布（2026-09-21）

本阶段只定稿中英更新日志并同步发布状态，复用下方死亡队友修复证据及 `ed6620a3` 的最终 Release 构建／最小 ZIP。分支和新标签原子推送、GitHub Release 创建及附件上传均由成功命令直接证明；现场 `.local/release-0.43.6/`，来源及限制见[发布记录](releases/0.43.6-PUBLISH.md)。本轮未重跑行为、构建、打包、远端复核或安装验收，发布状态不增加真实联机结论。
...
| 检查 | 本轮输入、结果与证据 |
|---|---|
| 真实候选池观察 | `--multiplayer-review-contracts window-selection --encounter FUZZY_WURM_CRAWLER_WEAK --beam 8 --nodes 180 --budget-ms 3000 --dop 1`，7 次内部固定节点搜索完成。`production-pools/window-selection.json` 分别记录展开、转移、生成深度、原最终池、共同周期、选中周期与离线深组；所有根未变，观察池模拟器均已释放，观察数与实际预算一致 |
| 晚回本控制 | `window-selection-payback`，相同参数但外部节点填 100；fixture 内部 52 节点、本机额外 1 点力量。`payback-pool-r2/window-selection.json` 完成。现役当前攻击和深组当前铺垫的第 3/7/14 周期敌方 HP 分别为 479/451/402 与 482/446/383；本机扣血均 0、两人存活。外部后续固定每回合至多一次攻击，没有真人队友行为 |
| 同分与药水口径 | 原无药 36 节点组深周期攻击/铺垫完全同分，不作为稳健质量收益；晚回本输入分开该因素。药水 180 节点组共同周期 1、展示 9 周期，深组后缀延迟用药；外部协议不消耗留存药水，因此末端 20 HP 差不构成资源匹配退化 |
| 一次入口失败 | `payback-pool/` 初次设置 52 节点触发原最低 100 限制，未运行 fixture。随后仅改外部设置为 100，内部固定 52；未放宽生产限制或延长超时 |
| 宿主构建 | `build-harness.log`、`build-payback.log` 各对应新增入口和后续追加输入，均 Release 0 警告/0 错误；第二次只跑受输入变化影响的晚回本案例，未重跑前七项。最后只将固定实验基线和加载程序集版本分开命名，`build-harness-final.log` 0 警告/0 错误，无重复行为测试 |
| 6 Pro 交付 | 269 次抽象搜索、24 个不同抽象根、18 项微型检查为外部执行；原报告/脚本/执行凭证已归档，完整 JSON/stdout 在 `pro-delivery/`。本地读原文、脚本和结果，没有重复运行，也不计为生产或原生通过 |
| 代表覆盖必要条件 | 对已记录八池直接计算每种完整当前回合的最大深度再取共同最小值，`docs/strategy/pro-window-selection-20260920/local-coverage-gate.json`。晚回本 4→5，药水 180 节点仍为 1；这是无选择缩减输入的必要条件检查，不是完整 C 门禁、动作游标或调度验收 |

这是生产候选观察和固定后续协议的研究，不是原生 actual/simulated 差分、完整战斗验收、真实联机或可见性能测试。没有为研究重建 Mod、打包、安装或发布。外部报告与本地原始数字分开归档，见[研究索引](strategy/pro-window-selection-20260920/README.md)。
...
| 层级 | 本轮执行与证据 |
|---|---|
| 源码 | 定点阅读最终资格/共同周期/终局、预览、完成池压缩、末父节点接收、根前用药与根后资源归属；固定发布输入与本地生产源码/项目及相关测试输入一致 |
| 本地生产函数探针 | 独立宿主引用既有 Release DLL，编译 1.54 秒、0 错误、1 条合成节点空值警告，执行退出码 0；5 组观察覆盖混合终局与截断、资源顺序、已知坏续行、尾部动作数和死亡控制，见[源码与输出](strategy/pro-review-0412-20260918/local-ordering-probe/README.md) |
| 外部抽象实验 | Pro 环境 Python 3.13.5，18 项预设断言与 6 个源码子串检查；独立 840 排列、216 三元关系，以及 24 节点乘 4 深度的 55,296 三元关系。原始[脚本](strategy/pro-review-0412-20260918/CombatSolver_0.41.2_abstract_checks.py)和[结果](strategy/pro-review-0412-20260918/CombatSolver_0.41.2_abstract_results.json)保留，本地未重复运行 |
| 文档 L0 | 本轮变更文档的本地路径/锚点、固定提交源码行号、引用定义/围栏、JSON/JSONL 与 Python 语法、维护文档空白；结果见[检查记录](strategy/pro-review-0412-20260918/archive-check.json)，原始回文和输出不作格式改写 |

本地探针只人工构造不可变候选事实并调用生产排序函数，没有捕获真实根、执行战斗模拟、最终搜索物化或双端网络。Pro 的 PASS 多数表示反例按预期存在，不表示修复已完成；其中玩具调度和给定数值例不证明生产可达性，细节见[证据限制](strategy/pro-review-0412-20260918/local-review.md#外部证据的实际范围)。没有重跑既有 840/216 C# 合同、七周期或 80 项单人对照，没有构建主 DLL、启动 Godot/Steam、验证原生差分、真实联机胜率或可见性能。
...
| 检查 | 本轮直接证据 |
|---|---|
| 失败基线 | 在原 DLL 的 `ExtractDeckRecursive` 入口注入原版托管 `RunState`，本机 ID 指向索引 1；实际返回索引 0 的卡组，`BASELINE_REPRODUCED local_seat=1 deck_owner=0` |
| 本机数据 | 修正 DLL 的三个实际入口直接调用原版 `LocalContext` 与模型属性；单人及二至四人、正反玩家顺序、每个座位共 20 组，逐项比较卡牌实例/顺序/重复项、血量二元组和遗物 ID |
| 生命周期边界 | 空卡组、拿牌后更新、返回列表隔离、遗物增减、其他玩家遗留缓存、零血量、未设置本机 ID、ID 不在列表时明确异常、零 ID、同 ID 换跑局以及无跑局均通过 |
...
真实主机/客户端联机、三/四人、全角色与第三方 Mod、可见布局和实机性能仍未验证。外部 Pro 的源码审查和抽象实验不替代游戏内实际/模拟差分。
...
| 检查 | 固定输入与本轮结果 |
|---|---|
| 多人能力策略隔离 | `--multiplayer-strategy-contracts`，双玩家、本机索引 1；新增 `INFLAME` 与额外攻击牌，覆盖本周期来伤后搜索，Beam 12 / 100 节点 / 1 秒 / DOP 1。未加隔离时投影 6 次、创建承诺 6 次、到期 11 次；修正后七项能力策略计数均为 0，仍选择能力牌加两次攻击，20 节点且仅一次建议搜索。`mp-before-guard-blocked/` 与 `mp-final/power-strategy-isolation.txt` |
| 原有多人扣血合同 | 同一请求继续完成九个案例、3 HP 增量等价、下一开始阶段完整原生对账、治疗/重算不刷新、额外回合/周期账本及 Fork 隔离；原双卡 Beam 2、已付血四卡 Beam 12，均 100 节点 / 1 秒 / DOP 1。新增隔离检查在内的整个合同步骤 1.577 秒，`mp-final/strategy-results.json` |
| 单人官方对照 | [五卡能力场景](../coverage/multiplayer/solo-power-compat.json)，Coordinator / Beam 12 / 每成员 350 节点 / 12 秒 / DOP 1；配置和节点统计按官方组合口径保留，不称为整请求 350 节点。两边均 1820 总展开、4583 总转移、17 动作、2 HP 战损。60 项非时间指标、17 个动作条目、根状态、目录身份以及完整 `route.json` 共 80 项一致、0 差异；包括普通/能力承诺成员和四条能力前缀路线的配置、顺序及结果。`solo-upstream/solo-power/`、`solo-merged/solo-power/`、`solo-comparison.json` |
| 七周期与全队状态 | `--multiplayer-contracts --encounter FOGMOG_NORMAL --beam 12 --nodes 350 --budget-ms 12000 --dop 2`，双方 500 HP 的既有长窗口输入；六次原生完整状态对账、五/七周期、23 动作/七回合、旧前缀重验 1 动作、四张多人卡、队友药水、额外回合、本人/队友选择、药水指令及手动操作边界均通过，合同步骤 8.340 秒，`mp-window/` |
| 手动启动 | `--multiplayer-start-contracts --encounter FOGMOG_NORMAL --beam 12 --nodes 100 --budget-ms 1000 --dop 1`；Host/索引 0、Client/索引 1、本机结果归属、动作等待恢复、三类启动失败与重试、四次建议搜索通过，合同步骤 1.823 秒，`mp-start/` |
| 构建与结构 | 当前主程序、宿主与独立官方基线 Release 编译均 0 警告/0 错误；Bash `REFACTOR_BOUNDARIES_OK search_files=196`，两端门禁增加相同的多人能力隔离声明。PowerShell 入口未执行 |
| 官方能力纯合同 | `PowerCardValuationChecks` 输出 `POWER_CARD_VALUATION_CHECKS_OK total=104 silent=17 ironclad=19 defect=20 regent=18 necrobinder=18 colorless=12`；这是登记/投影等纯合同，不扩大为 104 张牌的原生实战验收 |

离线对照工具适配官方新增的 `firstRoutePublishedMilliseconds`、`peakManagedHeapBytes` 及组合成员的实际耗时/分配/堆大小。成员的 `timeBudgetMilliseconds`、`nodeBudget`、展开量、战损与 `aggressivePowerCommitment` 继续比较，并用篡改这些字段的负向断言确认不会被过滤。本轮额外比较完整路线 JSON，包含目标和选择字段；未重跑已经成功的游戏模型请求来消除纯观测差异。

新增单人哨兵复跑时，向宿主传入只含 `generatedScenarioPath` 的请求 JSON，路径指向上述五卡场景，并使用 `--search-mode Coordinator --beam 12 --nodes 350 --budget-ms 12000 --dop 1`。当前请求保留为 `.local/upstream-0410-compat/solo-request.json`，其中本机绝对路径不提交。多人继续使用已有三个宿主入口；每个请求外层上限 120 秒，不使用真实游戏实例。

兼容改动范围的空白检查通过。完整合并差异有 40 处文件末尾空行警告，均来自原样合入的 `src/Search/PowerCardValuation/`；该目录与官方基线无差异，保留上游格式。

未验证真实主机/客户端网络、可见布局、三/四人组合、所有角色与第三方 Mod、完整自动部署及官方新增问题包的原生回放。上述步骤耗时包含不同合同工作，不能用于性能比较或胜率结论。结构化记录为 `MULTIPLAYER-UPSTREAM-0410-COMPAT`。
...
- 七回合扩展：`FOGMOG_NORMAL`、DOP 2，双方注入 500 HP 以覆盖长窗口；六回合完整原生状态相等，第五/第七个敌方回合按各自窗口停止。搜索返回 23 动作/7 回合建议，有效旧路线重新验证 1 个动作；1 节点合同提前返回并正确显示实际深度与上限。冻结根、RNG、四张卡牌、队友药水、额外回合、队友选择边界、本人选牌增量等价及用药政策均通过，产物 `.local/mp-seven/`。
- Runtime 入口合同：多人条件下自动计算、执行和全自动均关闭，自动回合请求不启动搜索；不依赖真实网络或 Godot UI。
- 最终单人哨兵对照未修改 `8411164` DLL：61 项搜索指标 + 9 个动作 + 根状态 + 目录字段共 72 项一致；126 节点、376 转移、T3 胜利、4 HP 累计损失。最终产物 `.local/solo-seven/`、`.local/solo-seven-comparison.json`；两项结构化记录为 `MULTIPLAYER-ADVISOR-MANAGED-CONTRACTS` / `MULTIPLAYER-ADVISOR-SOLO-SENTINEL`。
- 这是新增托管合同的局部语义证据；普通离线性能模式仍不能充当游戏内正确性验收。真实联机、可见 UI、三/四人和全量角色组合未测。宿主绕过、失败修正与产物路径见 [详细记录](multiplayer-advisor.md)。
- 发布定版仅调整版本和文档，行为源码仍为 `33525c9`，沿用本节已经取得的证据，不重复行为测试；安装包包含最终 Release DLL、manifest、Windows 辅助程序及两份许可文件。
