...
此目录维护用户提供的独立「尖塔军师」拿牌推荐模组的二进制修复。它不进入 CombatSolver 的编译或运行路径，也不修改 CombatSolver 的 0.41.2 版本。原 ZIP 保留在仓库根目录，解包材料与派生 DLL 位于 `.local/`，修复包位于 `releases/`；三者均由 Git 忽略，只有修复工具、验证和说明进入源码提交。
...
真实网络联机、Godot 可见界面、所有场景下的推荐结果和 Windows 实机未验证。通过的数据归属检查不等于多人推荐质量或胜率验收。

## 已交付产物

2026-09-18 从工具提交 `7ec629d` 完成 Release 构建并生成修正 DLL，随后一次性创建 `releases/尖塔军师(自用拿牌推荐)-v0.1.3.zip`（256,992 字节）。状态为本地修复包已交付；没有标签、远端推送或外部发布。成功后未重新解包或重复验证，原始输入与派生材料继续保留，统一归档状态见[开发笔记](../../docs/DEVELOPMENT_NOTES.md#归档状态2026-09-18)。
