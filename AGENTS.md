...
> **当前多人分支：** `codex/multiplayer-advisor` 基于官方 `d231e9e5 / 0.46.4`。单人保持该官方版本的原路径、参数和行为；fork 自有评分、保路、剪枝、预算和缓存只归多人政策/对象所有，公共文件仅允许显式多人接入。官方单人能力承诺不得进入多人候选；涉及公共战斗语义的依赖单列，不混入策略优化。现役行为见[多人军师](docs/multiplayer-advisor.md)。
>
...
>
> 发布状态统一见[版本索引](docs/releases/README.md)：当前基于官方 `0.46.4`，fork `0.46.5` 已发布，来源 `58c4c3d7`；分支、annotated tag 与 GitHub Release 已交付。官方标签不移动，同号日志单独归档。独立[尖塔军师修复工具](tools/SpireAdvisorMultiplayerFix/README.md)另有产物。历史批次、发布授权与证据归[开发笔记](docs/DEVELOPMENT_NOTES.md)和[策略索引](docs/strategy/README.md)，不作为新的操作授权。
>
> 本地准备版 `0.46.6` 支持多人烘焙手套耗尽前的手动计算，原生输入仍归玩家；暂停动作身份和队友准备边界必须成立，当前全队根只补完本机剩余准备。存在第三方回合开始扩展时明确拒绝该暂停入口。此版未创建标签或推送发布，验证见[测试矩阵](docs/TEST_MATRIX.md#0466fork多人烘焙手套选牌计算2026-09-25)。
...
- 单人行为保持既有路径；本分支已获用户授权增加多人手动军师。多人仅在手动重算时捕获全队状态和规划本机动作，不接管操作、不自动重算；范围与验证见 `docs/multiplayer-advisor.md`。
...
- 多人质量报告分开标明原生语义差分、合成脚本实验和真实联机证据。变体、重复种子与同根队友脚本不自动成为独立样本；测试侧的队友策略不得进入生产搜索。
- 多人实验的使用前提是只有本机玩家使用军师；队友独立决策。不能要求队友执行建议、等待本机铺垫或按本机评分配合；测试侧交错调度只覆盖行动先后，不构成产品里的队友控制。
