...
>
> 多人保持手动请求与手动操作、最多七个敌方周期、每周期 3 HP 本机扣血目标；治疗、手动重算和额外玩家回合不刷新额度。fork `0.41.1 / 5ad98a9` 已发布到 `sgpsdd-zyx/CombatSolver` 的任务分支、新标签和 GitHub Release；官方 `v0.41.0` 不移动。ChatGPT 6 Pro 两份回文与[本地复核](docs/strategy/pro-review-20260918/README.md)已归档；用户随后授权实施第一批，候选资格前置与同次选路共同周期已记入下一版本开发记录，仅本地提交。资源归属优先级和两步挑战仍待独立评审；研究回文不自动授权后续批次。禁止启动可见 Steam；无头结果不外推可见性能或联机胜率。
>
