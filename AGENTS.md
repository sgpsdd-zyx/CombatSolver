...
> **当前多人分支（2026-09-18）：** `codex/multiplayer-advisor` 已合入官方 `0e6cc2d / 0.41.0`，并在 `6e28485` 完成 [多人军师](docs/multiplayer-advisor.md) 的保路、共同周期比较和有效防御/救援改进。单人保持该官方版本的原路径、参数和行为；fork 自有评分、保路、剪枝、预算和缓存只归多人政策/对象所有，公共文件仅允许显式多人接入。官方单人能力承诺不得进入多人候选；涉及公共战斗语义的依赖单列，不混入策略优化。
>
> 多人保持手动请求与手动操作、最多七个敌方周期、每周期 3 HP 本机扣血目标；治疗、手动重算和额外玩家回合不刷新额度。当前 fork 定版为 `0.41.2`，行为源码为 `b29d6fc`：候选资格前置并贯通同次选路的共同周期。用户已发出“发布”，本次沿用 `sgpsdd-zyx/CombatSolver` 任务分支、新标签和 GitHub Release 的 fork 渠道；官方 `v0.41.0` 不移动。ChatGPT 6 Pro 两份回文与[本地复核](docs/strategy/pro-review-20260918/README.md)已归档；资源归属优先级和两步挑战仍待独立评审，研究回文不自动授权后续批次。禁止启动可见 Steam；无头结果不外推可见性能或联机胜率。
>
