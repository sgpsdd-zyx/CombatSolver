...
>
> 多人保持手动请求与手动操作、最多七个敌方周期、每周期 3 HP 本机扣血目标；治疗、手动重算和额外玩家回合不刷新额度。本批沿用“下一版本（fork，开发中）”记录；用户本次授权归档并推送至 `sgpsdd-zyx/CombatSolver`，仅同步任务分支与已有当前版本标签。安装包定版和渠道发布仍按第 9 节口令执行。禁止启动可见 Steam；无头结果不外推可见性能或联机胜率。
>
