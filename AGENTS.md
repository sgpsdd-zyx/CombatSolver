...
> **当前分支（2026-09-17）：** 用户授权实现多人手动军师并做适当模拟测试，随后要求扩大预测至 5～7 回合。本分支默认最多推演七个敌方回合，只在手动重新计算时捕获全队状态并规划本机出牌/用药，不自动操作或重算，保持单人原路径。用户随后明确授权推送至 `sgpsdd-zyx/CombatSolver` fork 并提供可直接安装的 GitHub Release，本次定版为 `0.40.3`；该授权结束本分支此前不提升版本、不发包的批次。实现与验证见 [多人军师](docs/multiplayer-advisor.md)。本次发布范围仅为该 fork 的分支、版本标签和 GitHub Release；不启动可见 Steam，不发布上游创意工坊或夸克网盘。
