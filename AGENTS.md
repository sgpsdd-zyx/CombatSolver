...
> **当前多人策略批次（2026-09-17）：** 基于已合入官方 `0e6cc2d / 0.41.0` 的 `codex/multiplayer-advisor`，实施 [多人军师](docs/multiplayer-advisor.md) 中的保路、共同周期比较和有效防御/救援改进。单人保持该官方版本的原路径、参数和行为；fork 自有评分、保路、剪枝、预算和缓存只归多人政策/对象所有，公共文件仅允许显式多人接入。官方单人能力承诺不得进入多人候选；涉及公共战斗语义的依赖单列，不混入策略优化。
>
> 多人保持手动请求与手动操作、最多七个敌方周期、每周期 3 HP 本机扣血目标；治疗、手动重算和额外玩家回合不刷新额度。本批沿用“下一版本（fork，开发中）”记录，本地验证后提交，不从研究实施扩展到发布。后续发布按第 9 节口令执行。禁止启动可见 Steam；无头结果不外推可见性能或联机胜率。
>
