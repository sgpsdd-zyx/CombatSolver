...
>
> 多人保持手动请求与手动操作、最多七个敌方周期、每周期 3 HP 本机扣血目标；治疗、手动重算和额外玩家回合不刷新额度。用户本次要求先发布最新 fork，再交 ChatGPT 6 Pro 审查并归档研究；本批定版为 `0.41.1`，仅发布 `sgpsdd-zyx/CombatSolver` 的任务分支、对应新标签和 GitHub Release，沿用本分支的渠道边界。官方 `v0.41.0` 不移动。研究回文不等于批准实施；后续行为改动另记开发批次。禁止启动可见 Steam；无头结果不外推可见性能或联机胜率。
>
