...
>
> 多人保持手动请求与手动操作、最多七个敌方周期、每周期 3 HP 本机扣血目标；治疗、手动重算和额外玩家回合不刷新额度。fork `0.41.1 / 5ad98a9` 已发布到 `sgpsdd-zyx/CombatSolver` 的任务分支、新标签和 GitHub Release；官方 `v0.41.0` 不移动。ChatGPT 6 Pro 两份回文与[本地复核](docs/strategy/pro-review-20260918/README.md)已归档，候选资格/共同周期管线及资源归属发现尚未实施。研究回文不等于批准实施；后续行为改动另记开发批次。禁止启动可见 Steam；无头结果不外推可见性能或联机胜率。
>
