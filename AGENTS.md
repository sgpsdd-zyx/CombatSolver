...
> **2026-09-23 本地准备 `0.44.2`：** 多人将原生无来源实际扣血按阶段人数折算参与选路，个人贡献信用/达标仍保留真实归属。三类伤害计数随根、Fork、周期观察和政策标签保存；不改实际结算，不预测队友输出。队友死亡折价与威胁压力原型已撤回，十四周期及预算不变，详见[实验与采用](docs/strategy/multiplayer-shared-damage-20260923/implementation.md)。尚未创建标签、推送或发布。

> **2026-09-22 已发布 `0.44.1`：** 本机贡献策略与官方0.44.0合并，来源 `86026b05`，GitHub分支/annotated tag/Release已发布；见[发布记录](docs/releases/0.44.1-PUBLISH.md)。32项贡献合同、用药排序、单人哨兵和原生UI通过；真实联机与可见性能仍未验收，不保证十四周期最优或普遍正收益。先前0.43.6的死亡队友修复证据保留于[测试矩阵](docs/TEST_MATRIX.md#0436fork死亡队友重算2026-09-21)。
