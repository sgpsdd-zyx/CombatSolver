...
> **多人策略追加（2026-09-17）：** 用户确认计算恢复正常，并授权采用「每回合可以接受 1～3 HP 损失，在此范围内尽量输出」的策略。`0.40.5` 按每个敌方周期最多 3 HP 的目标排序，致命风险优先避让；当回合已经付出的扣血计入额度，治疗、手动重算和额外玩家回合不刷新额度。仅调整多人搜索、保路及说明，保留七回合窗口与单人原路径。完成本地提交和最小 ZIP 准备；本次普通功能请求不新增远端推送、标签或渠道上传。证据见 [多人军师](docs/multiplayer-advisor.md)。
